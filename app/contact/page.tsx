...
export default function ContactPage() {
  ...
  const refreshCaptcha = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCaptchaUrl('') // Clear the URL on failure
      }
    } catch (error) {
  ...
  const refreshNewsletterCaptcha = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Newsletter captcha load failed:', response.status)
        setNewsletterCaptchaUrl('') // Clear the URL on failure
      }
    } catch (error) {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log("Newsletter subscription attempt:", { email: newsletterEmail, captcha: newsletterCaptcha })
      ...
      if (data.success) {
        alert("Newsletter obunasi muvaffaqiyatli! 🎉")
        setNewsletterEmail("")
        setNewsletterCaptcha("")
        refreshNewsletterCaptcha()
      } else {
        alert(data.message || "Obuna bo'lishda xatolik yuz berdi")
        setNewsletterCaptcha("")
        refreshNewsletterCaptcha()
      }
    } catch (error) {
